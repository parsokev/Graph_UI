...
int find_shortest_path(const std::string& s_vertex, const std::string& des_vertex, const std::string& graph_filename, const std::string& path_filename, main_hashmap<double>&& adj_list) {
    ...
    if (s_vertex.compare(des_vertex) == 0) {
        ...
        samePath.append("\nNOTE: Path Image Generation will be disabled since submitted path connects same verticies");
        sameDest.setText(samePath);
        sameDest.exec();
    ...
    for (const auto& key : shortest_path) {
        if (key == shortest_path.back() ) {
            shortPathMsg.append(key);
        } else {
            shortPathMsg.append(key).append(" -> ");
        }
    }
    shortPathMsg.append('\n');
    ...
    pathMessage.setText(shortPathMsg);
    pathMessage.exec();
    return 0;
}
...
int find_MST(std::string& source_vertex, const std::string& graph_filename, const std::string& MST_filename, main_hashmap<double>&& adj_list) {
    ...
    for (unsigned int i = 0; i < MST_edges.size(); i++) {
        ...
        mstMsg.append(source).append(" <----> ").append(dest).append('\n');
        if ( i > 15){
            mstMsg.append("...\n");
            break;
        }
    ...
    QString sum = sum.number(MST_sum);
    mstMsg.append('\n').append("TOTAL COST/DISTANCE: ").append(sum).append("\n\n");
    mstPath.setText(mstMsg);
    std::cerr << '\n';
